...
- `DashboardModel.swift` — consumes `RaceSnapshot` stream (drop stale by seq no.); exposes meter %, lives, progress, speed, phase; sends `.boost` (with retry+token) and `.reactionCam(on:)`.
