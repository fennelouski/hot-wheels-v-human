...
- `DashboardModel.swift` — consumes `RaceSnapshot` stream (drop stale by seq no.); exposes meter %, lives, progress, speed, phase; sends `.boost` (with retry+token) and `.reactionCam(on:)`. `submit` sends the ranked track draft (`.trackBlueprint(rank:ownerID:)` per pick) and caches the kid's own design in `myDesign` for the lobby's car swatch.
- `ConnectionLadder` (in `DashboardView.swift`) — the pre-race "what do I do now?" checklist. Backdrop is `App/LobbyBackground.swift` (shared with the TV lobby), driven by an `energy` DashboardView computes from `model.transportState` + `model.readySent` — the iPad only sees its own connection, not the other iPad's, so its energy reads searching → connected → ready off itself alone.
