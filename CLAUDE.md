...
- Ships to iPad + Apple TV (+ Vision) only — `TARGETED_DEVICE_FAMILY = "2,3,7"`. iPhone is out: the workshop UI is sized for an iPad.
...
- Build must stay green on BOTH destinations (commands in XCODE-SETUP.md §8). Run tests before declaring any phase done.
...
## Guardrails
- Don't rename the app/bundle IDs, the scheme, or the `hwvh-race` service. The trademark rename (PRD §1.1) happened at the *display* layer only: release builds show **HWvH** everywhere a player can read, debug builds show "Hot Wheels vs Humans". On-screen text goes through `AppBranding.name`; the Home Screen label is `INFOPLIST_KEY_CFBundleDisplayName`, set per configuration. Never hard-code either name in a view.
